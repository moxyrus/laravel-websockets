...
class WebSocketStatisticsEntriesController
{
    public function store(Request $request)
    {
        ...
        $tableStatisticModel = config('websockets.statistics.model');

        collect($validatedAttributes['chats'])->each(function ($chat) use ($tableStatisticModel){
            $tableStatisticModel::create($chat);
        });
